...
class CitizenAgent:
    ...
    def decide(self, economy: 'EconomySystem', buildings: List[Building], world_width: int, world_height: int):
        ...
        elif self.state.wealth < 15:
            self.state.current_goal = "work"
            self._seek_building(economy, buildings, BuildingType.OFFICE, world_width, world_height)
        elif self.state.wealth >= 30 and self.state.energy < 70:
            self.state.current_goal = "shop"
            self._seek_building(economy, buildings, BuildingType.MARKET, world_width, world_height)
        else:
    ...
    def _random_move(self, w: int, h: int):
        ...
        self.state.energy = round(max(0.0, self.state.energy - 0.2), 2)
